...
const authCookies = ['redirect_after_login'];
...
// =========================================================================
// 6.  MOBILE MENU (pure DOM, but called after DOM ready)
// =========================================================================
...
// =========================================================================
// 6.  MOBILE MENU (fallback if Bootstrap is missing)
// =========================================================================
...
// =========================================================================
// 7.  MODAL CLASSES (instantiated after DOM ready)
// =========================================================================
let formModal = null;
...
function populateUserProfile() {
  ...
  const pointBal = user.point_bal || '0';
  const trustScore = user.trusting_score || '0';
...
// =========================================================================
// 11. NAVIGATION INJECTION (DOM dependent)
// =========================================================================
    ...
    <header class="site-header fixed-top" id="mainNav">
      <nav class="navbar navbar-expand-lg" aria-label="Main navigation">
        <div class="container header-inner">
          ...
          <ul class="nav-item dropdown d-md-none">
            ...
            <ul class="dropdown-menu dropdown-menu-start shadow-sm border-0 rounded-3 mt-2" 
              ...
              <li><hr class="dropdown-divider"></li>
              <div id="mobileNotificationItems"></div>
            </ul>
          </ul>
...
(function injectNotificationStyles() {
  ...
  style.textContent = `
    ...
    .nav-item.notification-item-nav { margin-right: 0.75rem; }
    .notification-link {
      display: block;
      cursor: pointer;
      padding: 0.25rem 0;
    ...
    .notification-item.unread { border-left: 3px solid var(--accent, #2c7a5e); }
    ...
    .notification-item .delete-notif-btn:hover { opacity: 1; }
    .dropdown-item-text.fw-semibold { padding: 0.5rem 1rem; }
  ...
  if (!document.getElementById('dashboard-breadcrumb-styles')) {
    ...
    style.textContent = `
      ...
      @media (max-width: 576px) {
        .dashboard-breadcrumb-shell { padding-top: 68px; }
        .dashboard-breadcrumb { min-height: 44px; font-size: 0.82rem; }
        .dashboard-breadcrumb .breadcrumb-back-btn { padding: 0.3rem 0.65rem; }
      }
    `;
    document.head.appendChild(style);
  }
  ...
  reportBtn.addEventListener('click', (e) => {
    ...
    formModal.open({
      ...
      formHtml: `
        ...
        <hr class="my-4">
        <h6 class="fw-bold">🔐 Owner verification questions</h6>
        <p class="small text-muted">When someone claims this item, we'll ask them the same questions. If answers match, we'll share your contact details.</p>
        <div class="form-floating-label">
    ...
    // Set up drag & drop after modal open
    setTimeout(() => {
...
async function refreshPointBalance() {
  ...
  try {
    ...
    const mobilePointsBalance = document.getElementById('mobilePointsBalance');
    if (mobilePointsBalance) mobilePointsBalance.textContent = newBalance;
...
// =========================================================================
// 17. OPTIONAL FEATURES (AOS, Particles, Smooth Scroll)
// =========================================================================
...
// =========================================================================
// 18. NETWORK ERROR OVERLAY (injected globally)
// =========================================================================
(function () {
...
function initAll() {
  ...
  // Inject navigation (creates all nav elements)
  injectNavigation();

  // Initialize modals (if the containers exist)
  if (document.getElementById('formModal') && !formModal) {
    formModal = new FormModal();
  }
  if (document.getElementById('confirmModal') && !confirmModal) {
    confirmModal = new ConfirmModal();
  }

  // Initialize features that require DOM
  initOptionalFeatures();
  initScrollToTop();
  initMobileMenu();          // fallback if Bootstrap missing
  initMobileMenuOutsideClick();
  initReportLostFeature();

  // Fetch notifications (protected, but will be guarded by auth)
  fetchAndRenderNotifications();

  // Initialize tooltips after all DOM updates
  setTimeout(initTooltips, 200);

  // Attach refresh listeners if they exist
  const refreshBtn = document.getElementById('refreshPointsBtn');
  if (refreshBtn) refreshBtn.addEventListener('click', refreshPointBalance);
  const refreshSummaryBtn = document.getElementById('refreshSummaryPointsBtn');
  if (refreshSummaryBtn) refreshSummaryBtn.addEventListener('click', refreshPointBalance);
}
